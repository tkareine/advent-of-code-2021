    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if components.len() == 2 {
            let direction: FoldDirection = components[0]
                .parse()
                .map_err(|e| ParseFoldInstructionError::UnexpectedDirection(e))?;
            let line_position: usize = components[1].parse().map_err(|e: ParseIntError| {
                ParseFoldInstructionError::UnexpectedLinePosition(e.kind().clone())
            })?;
            Ok(FoldInstruction {
                direction,
                line_position,
...
impl FromStr for DotPaperComponent {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s.starts_with(fold_instruction_key) {
            let fold_instrution: FoldInstruction =
                s[fold_instruction_key.len()..]
                    .trim()
                    .parse()
                    .map_err(|e| ParseDotPaperComponentError::ParseFoldInstructionError(e))?;
            Ok(DotPaperComponent::FoldInstruction(fold_instrution))
        } else {
            let point: Point = s
                .trim()
                .parse()
                .map_err(|e| ParseDotPaperComponentError::ParsePointError(e))?;
